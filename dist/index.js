...
// Set up routes
app.use("/api/games", gameRoutes_1.default); // Mount game routes under '/api/games'
app.get("/", (req, res) => {
    res.json({
...
io.on("connection", (socket) => {
        ...
        try {
            ...
            if (game) {
                ...
                if (game?.players.length == 1) {
                    await Game_1.default.findOneAndRemove({
                        _id: new mongoose_1.Types.ObjectId(game?._id),
                    }).lean();
                    return;
                }
                    ...
                    const updatedGame = await Game_1.default.findOneAndUpdate({
                        _id: new mongoose_1.Types.ObjectId(game?._id),
                    }, {
                        $set: {
    ...
    socket.on("rook-moved", async (data) => {
        try {
            ...
            const updatedGame = await Game_1.default.findOne({
                _id: gameId,
            }).lean();
            if (!updatedGame) {
                throw new Error("Game Not Found");
            ...
            if (updatedGame) {
                const otherPlayer = updatedGame.players.find((player) => player.socketId != socketId);
                const currentPlayer = updatedGame.players.find((player) => player.socketId == socketId);
                if (!otherPlayer || !currentPlayer) {
                    throw new Error("Player not found");
                }
                if (rookRow == 7 && rookCol == 0) {
                    await Game_1.default.findOneAndUpdate({
                        _id: gameId,
                    }, {
                        $set: {
                            "gameState.isGameCompleted": true,
